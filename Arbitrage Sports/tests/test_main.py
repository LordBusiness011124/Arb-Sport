...
def test_build_matchable_kalshi_market_accepts_market_title_winner_shape() -> None:
    now = datetime(2026, 4, 20, 0, 0, tzinfo=UTC)
    snapshot = KalshiMarketSnapshot(
        ticker="KXNCAAB-title-shape",
        event_ticker="KXNCAAB-title-shape",
        event_title="North Carolina vs Duke",
        event_sub_title="April 20, 2026",
        market_title="Will North Carolina beat Duke?",
        series_ticker="KXNCAAB",
        category="Sports",
        status="active",
...
def test_infer_kalshi_league_accepts_pro_basketball_series_prefix() -> None:
    now = datetime(2026, 4, 20, 0, 0, tzinfo=UTC)
    snapshot = KalshiMarketSnapshot(
        ticker="KXBBLGAME-1",
        event_ticker="KXBBLGAME-1",
        event_title="Boston Celtics vs New York Knicks",
        event_sub_title="Boston Celtics vs New York Knicks",
        market_title="Will Boston Celtics beat New York Knicks?",
        series_ticker="KXBBLGAME",
        category="Sports",
        status="active",
        fetched_at=now,
        occurrence_time=datetime(2026, 4, 20, 1, 0, tzinfo=UTC),
        ...
        rules_primary="If Boston wins the game, then the market resolves to Yes.",
        yes_ask_levels=(KalshiOrderLevel(price=0.50, size=100.0),),
        no_ask_levels=(KalshiOrderLevel(price=0.50, size=100.0),),
        yes_bid=0.48,
        yes_bid_size=100.0,
    ...
    assert _infer_kalshi_league(snapshot) == "nba"
